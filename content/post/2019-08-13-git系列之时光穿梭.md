...
前面说了往git仓库中加文件需要两步走，第一步add 第二步 commit,其实这里：第一步是用git add把文件添加进去，实际上就是把文件修改添加到暂存区；
...
管理修改 git 管理的是修改本身，而不是文件。这也是git的特色之一。
...
删除文件 假如我已经提交的文件，我本地给删除了。如何git库中也进行删除呢？
